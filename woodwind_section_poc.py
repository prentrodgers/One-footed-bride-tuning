...
# sway amplitude, same technique as the trombone slide in
# brass_section_poc.py, rather than an instant on/off.
...
# Which of each draw_*()'s returned patches share the family's base body
# colour and should join the playing-glow (indices into that patch list).
# clarinet/oboe's single-block bodies already glow fully with just index 0;
# horn/bassoon's bodies are two same-coloured pieces (coil rings / tubes) —
# lighting only one left the other looking unlit.
GLOW_PATCH_IDX = {
    'clarinet': (0,),
    'oboe':     (0,),
    'horn':     (0, 1),   # outer + middle coil
    'bassoon':  (0, 1),   # left + right tube
}
...
class WoodwindScene:
    ...
    def __init__(self, fig, ax, seats):
        ...
        self._body_patches  = {}   # sid → list of glow-colour patches (see GLOW_PATCH_IDX)
    ...
    def update(self, t, states):
        for s in self.seats:
            ...
            # ── Sway: slow sinusoidal idle rock, faded by an envelope that
            # eases toward 1 (playing) or 0 (been silent a while) — an
            # instrument settles to stillness after resting, rather than
            # swaying forever, and eases back into motion once it resumes.
